...
function pkcs1pad2(s,n) {
    ...
    for (i=0; i<128; i++)
        ba[i] = i< s.length ? s.charCodeAt(i) : 0;
...
function decrypt(base64) {
    var s="";
    jQuery.ajax({
        url:"/factory/decrypt?arg="+base64,
        success:function (result) {
            s=result;
        },
        dataType:"text",
...
function doRSAEncrypt() {
    ...
    {
        var rc4 = rc4stream("1234567890123456");
        var res = rc4("this is the test");
        var enc = Base64.encode(res);

        var back = decrypt(enc);
    }
   ...
   var ciphertext  = linebrk(res, 64);
    var cipherb64 = linebrk(hex2b64(res), 64);
    var foo = "";

    setCipher(cipherb64);

    var rc4 = rc4stream(text);
    var rc41 = rc4stream(text);

    var enc = rc4("this is the message");
    log(enc);
    var enc = rc41(enc);
    log(enc);
