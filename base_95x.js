...
// UNIHAN ADD END
    IPV6:true,
//    Languages:["English:English", "Spanish:Espanol", "French:Francais", "German:Deutsch","Portuguese:Portugu̻s do Brasil","Dutch:Dutch","Romanian:Romanian","Italian:Italiano","Polish:Polish","Hungarian:Hungarian","Czech:Czech","Slovakian:Slovakian"],
    Languages:["English:English", "Spanish:Espanol", "French:Francais", "German:Deutsch","Portuguese:Portugu̻s do Brasil","Polish:Polish"],
    Moca: "",
    Wifi: "",
    //SERCOMM ADD
    muti: "",
...
// PEGATRON ADD START - PD12331
function getMocaVersion()
{
	var mocaModel = attrs["Moca"];
	return mocaModel;
}
// PEGATRON ADD END - PD12331
...
function hasWiFi80211ac()
{
    ...
    if ( (WifiModel == undefined) || (WifiModel == "") )
        return false;

    WifiModel = WifiModel.asInt();
    if ( (WifiModel == 5 /* QCA_AC_DBC */) ||
         (WifiModel == 7 /* MTK_AC_DBC */) ||
         (WifiModel == 8 /* MTK7615_AC_DBC */) )
    {
        return true;
    }
    return false;
...
function isMTKDBC()
{
    var WifiModel  = attrs["Wifi"];
    if( (WifiModel != undefined) && (WifiModel != "") &&
        ((WifiModel.asInt() == 7 /* MTK_AC_DBC */) || (WifiModel == 8 /* MTK7615_AC_DBC */)) )
    {
        return true;
    }
    return false;
...
// UNIHAN ADD END
function isMTK7615DBC()
{
    var WifiModel  = attrs["Wifi"];
    if( (WifiModel != undefined) && (WifiModel != "") &&
        (WifiModel.asInt() == 8 /* MTK7615_AC_DBC */) )
    {
        return true;
    }
    return false;
...
function isCELENO()
{
    var WifiModel  = attrs["Wifi"];
    if( (WifiModel != undefined) && (WifiModel != "") &&
        ((WifiModel.asInt() == 3 /* CELENO_DBC */) || (WifiModel == 4 /* CELENO_24_CLR260 */) || (WifiModel == 6 /* CELENO_24_CLR260H */)) )
    {
        return true;
    }
    return false;
...
function showSSIDGraph() {
    ...
    /* Task 55026 - IZZI ADD */
    if( isSettingsSameAsNA() || isAltice() ) return true; // PEGATRON MOD - PD10988
...
function showMacBridgeGUI() {
    ...
    if ( isSettingsSameAsNA() && attrs["ModelName"]&& !attrs["ModelName"].contains("DG860") ){// Only support for NA, exclude model 862.
        return true;
    }
    if(isNET()) {
        return true;
...
function showL2TPGUI() {
	if(!isLoggedIn()) {
        ...
        // ARRIS MOD START, remove the strict of supporting only Europe
	//if (attrs["ModelName"]&& attrs["ModelName"].contains("S") ){// Only support Europe (S).
		return true;			
	//}
...
function getLangCode(){
	var def_lang = {"Spanish":"es", "French":"fr", "German":"de", "Portuguese":"pt-br", "Dutch":"nl", "Romanian":"ro", "Italian":"it", "Polish":"pl", "Hungarian":"hu", "Czech":"cz", "Slovakian":"sk","Turkish":"tk"};
	var lang = language();
...
// UNIHAN ADD END
        ...
        // FOXCONN ADD START FOR PD.40311
        if (isNET()){
            attrs["isWifiAccess"] = o["isWifiAccess"];
        }
        // FOXCONN ADD END
        }
    }
    ...
    // FOXCONN ADD START FOR PD.40311
    if(isNET() && attrs["isWifiAccess"] == "deny"){
                return '5';
    }
    // FOXCONN ADD END
...
function login(name, password) {
    ...
    jQuery.ajax({
        ...
        success:function (result) {
            result = decrypt(result);
            // ARRIS MOD START
            if (Base64.decode(result).indexOf("DefPSK") != -1)
            {
                useDefPSK = true;
            }
            else
...
function getNonce() {
    ...
    if (!n) {
        n = "_n="+(""+Math.random()).substr(2,8); //ARRIS MOD, PD68505
...
{
    if (attrs["conType"] && attrs["conType"].contains("EROUTER"))
    {
        return true;
    }
    else
