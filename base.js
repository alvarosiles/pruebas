...
var basePath = "";
var _lastEvent;
// FOXCONN ADD START - PD.37310
var resetDeviceNeeded=false;
// FOXCONN ADD END
...
// UNIHAN ADD START
// PEGATRON ADD START PD4668
function setMoCANodeSearch(val) {
    setSessionStorage("ar_mocanode_search", val);
}
function isMoCANodeSearch() {
    if (getSessionStorage("ar_mocanode_search") === undefined || !getSessionStorage("ar_mocanode_search")) {
        setMoCANodeSearch(0);
        return 0;
    }
    return getSessionStorage("ar_mocanode_search").asInt(0);
}
// PEGATRON ADD END
...
function isLGI() {
    return ((customerId() === 7) || (customerId() === 8) || (customerId() === 9) || (customerId() === 20));
}
function isVM() {
    return customerId() === 8;
}
function isRetail() {
    return customerId() === 24;
}

// PEGATRON ADD START - PD11406, PD11407, PD11409, PD11410, PD11412
function isCox() {
    return customerId() === 5;
}
// PEGATRON ADD END - PD11406, PD11407, PD11409, PD11410, PD11412
function isNA() {
    return customerId() === 1;
}
// FOXCONN ADD START - PD42308, PD42311
function isNET() {
    return customerId() === 25;
}
// FOXCONN ADD END - PD42308, PD42311
...
/* Task 55026 - IZZI ADD */
function isIZZI() {
    return customerId() === 36;
}

function  isSettingsSameAsNA() {
    var custID = customerId();
    switch( custID )
    {
    case 1:    //CUSTOMER_DEFAULT
    case 5:    //CUSTOMER_COX
    case 33:   //CUSTOMER_CVARGENTINA
    case 34:   //CUSTOMER_UNE
    case 35:   //CUSTOMER_TIGO
    case 36:   //CUSTOMER_IZZI
    case 45:   //CUSTOMER_CLARO_NICARAGUA
    case 46:   //CUSTOMER_CLARO_GUATEMALA
        return true;
        break;
    default:
        return false;
...
//PEGATRON ADD START for PD42515
function first_login_status() {
    if (getSessionStorage("ar_needchange_password") === undefined || getSessionStorage("ar_needchange_password") === null) {
        setSessionStorage("ar_needchange_password",  snmpGet1(arEnforceChangePassword.oid+".1") || "0");
    }
    return getSessionStorage("ar_needchange_password");
}

function reset_first_login_status() 
{
    setSessionStorage("ar_needchange_password",  snmpGet1(arEnforceChangePassword.oid+".1") || "0");
    return getSessionStorage("ar_needchange_password");
}

function is_password_using_wpakey() {
    if (getSessionStorage("ar_password_using_wpakey") === undefined || getSessionStorage("ar_password_using_wpakey") === null) {
        setSessionStorage("ar_password_using_wpakey",  snmpGet1(arPasswordUsePSKKey.oid+".1") || "0");
    }
    return getSessionStorage("ar_password_using_wpakey");
}

function reset_password_using_wpakey_status()
{
    setSessionStorage("ar_password_using_wpakey",  snmpGet1(arPasswordUsePSKKey.oid+".1") || "0");
    return getSessionStorage("ar_password_using_wpakey");
}
//PEGATRON ADD END for PD42515
...
function loadMenus() 
{
	...
	// UNIHAN MOD END PROD00221873 
        ...
        if(isLGI())
        {
            ...
            if(parseInt(arLanPassThru.get(200)) == 1)
            {
                var gwFields = ";hide:ParentalControl;hide:ConnectedDevices;hide:AdvancedSettings;hide:home_wizardfields;hide:home_conn_devices_field";
                table.push([gwFields]);
                if( isVM() )
                {
            ...
            {
                var IPv6fields = ";hide:device_status_WANIPv6Info;hide:device_status_WANDSLiteInfo;hide:dhcp_setting_DHCPv6server;hide:firewall_IPv6Firewall;hide:ipfilter_IPv6Filter";
                table.push([IPv6fields]);
            }
            else if(parseInt(IPMode) == 2) // IPv6 mode
            {
                var IPv4fields = ";hide:device_status_WANIPv4Info;hide:dhcp_setting_DHCPv4;hide:firewall_IPv4Firewall;hide:ipfilter_IPFilterList";
                table.push([IPv4fields]);
            }

	    //Hide changepassword page in case of CSR login or for login from gateway interface
...
function inlineButtonImage(label1, image, onclick) {
    ...
    return _tr("id:tr_"+label1,_td("width:35%", "text:{{"+label1+"}}"), _td(_input("type:image", "id:" + label1, "src:" + image, onclick), helpTag(label1)));
    // PEGATRON MOD END PD46052
}
...
function rotext(label1, fmt) {
    if (technicianOnly(label1) && !isTechnician())
        return null;
    // PEGATRON ADD START PD46052
    if (!fieldsetVisible(label1))
        afterBuild(function() { $("#tr_" + label1.word()).hide(); });
    if  (!fieldsetEnabled(label1))
        afterBuild(function() { $("#" + label1.word()).attr('disabled', true); });
    // PEGATRON ADD END PD46052
    var f = fmt ? fmt : formatter();
    var l = parseLabel(label1);
    ag[label1] = f.load ? f.load(ag[label1]) : ag[label1];
    // PEGATRON MOD START PD46052
    return _tr("id:tr_"+label1,_td("width:35%", "text:{{" + label1 + "}}"), _td(_input("type:text", "id:" + l.label, (f.size ? "size:" + f.size : null), "class:read_only", "readonly:readonly", "value:" + htmlEscape(ag[label1])), helpTag(label1))); // UNIHAN MOD, PROD00216938
    // PEGATRON MOD END PD46052
}
...
function formHelpText(label1, label2, helpText){
    helpText = xlate(helpText);
    return _div(_div("class:description", _h3("text:{{" + label1+"}}"), _div("text:{{" + label2+"}}", "style:display: inline"), helpTag(label1, helpText)),
...
function select(label1, vals, onchangefunc, selectedValue, helpText) {
    ...
    var options = _.map(vals, function f(v, index) {
        var txt=v.substring(v.indexOf(":")+1);
        v = v.split(":");
        if (v[0] == value || (!haveSelected && index ===0))
            return  _option("value:" + v[0], "text:" + htmlEscape(txt), "selected:selected");
        else return  _option("value:" + v[0], "text:" + htmlEscape(txt));
    });
    afterApply(function() {
        if (ag[label1] !== undefined) {
            var v = $("#" + label1).valOrChecked();
...
function select2Optional(label1, vals, onchange, sepText, optionalCheckText) {
    ...
    return _tr(_td("width:35%", "text:{{" + label1+"}}"),
            _td(_div("style:height:24px;width:auto;", _input("id:" + l.label + "_Checked", "type:checkbox", "checked:checked"
                ,
                  function onclick() {
...
function getDefaultPage(){
	...
	{
		defPage = ((!isTechnician())&&(first_login_status()=="1"))?"basic_password":"basic_setup";
	}
	//PEGATRON ADD END for PD42515
	return defPage;
}
...
// UNIHAN MOD END
    ...
    //PEGATRON ADD START PD4668
    if ( subMenu.id != "MocaNodeList")
    {
        setMoCANodeSearch(0);
    }
    //PEGATRON ADD END PD4668
...
function DoApply(args) {
	try{
        ...
        if (WaitDialog._isOpened) // ARRIS MOD FOR CLM34044
...
//PEGATRON ADD START for PD64899
function ApplyPasswdChangeButton() {
    return buttons("{{Apply}}", function onclick(event) {
        Apply({delayClose:3000});
});
}
//PEGATRON ADD END for PD64899
...
function loginbuild() {
    ...
    doLogin = function() {
        clearSessionStorage();
        // ARRIS MOD START
        var retCode = login($("#UserName").val(), $("#Password").val())
        //FOXCONN ADD FOR PD.42311 
        if (isNET() && isLoggedIn() == '5'){
        
            alert(xlate("Access from wireless devices is forbidden. Please use a device connected over Ethernet to manage your gateway!"));
            clearLoginCredential();
        }
        //FOXCONN ADD END
        //SERCOMM ADD
        //PEGATRON ADD START for PD42515
        var failMsg="Invalid user name or password!";
        var additionInfo="";
        //PD75393, MOD SATRT, Removing unused part
        //if((!isTechnician())&&(is_password_using_wpakey() == "1"))
        //{
        //    additionInfo="\nPlease use default WPA key as password to login.";
        //}
        //PD75393, PEGATRON MODIFY END
        //PEGATRON ADD END for PD42515
        if (retCode == '6')
        {
            if (isCox())
            {
                alert(xlate("Invalid user name or password!\n\nPlease enter the last 13 digits of the serial number (sometimes referred to as \"SN\") that is printed on the label for this device into the password field."));
            }
            else
            {
        ...
        // ARRIS MOD END
            ...
            alert(xlate(failMsg+additionInfo)); //PEGATRON MOD for PD42515
    ...
    return form("Login", (isSuddenlink() || isAltice()) ? "LoginText_sl" : "LoginText",
            fieldset("Login",
                    [text("UserName"),
                        text("Password")
                    ]),
            ...
            _br(),
            _br(),
            _div("style:border-top:1px solid #ccc;", _tr(_td("text:" + xlate("OssInfoText")) , link("OSSLink", "OSS.html", xlate("OssInfo")), _td("text:" + ".")))
            );
}
...
function getSelectedBss() {
    if (EnablePlumeAgent())
        return getSessionStorage("ar_selected_bss") || getPlumeBss()[0];  //PEGATRON MDD for PD 88592
...
function isBSEnabled()
{
    // PEGATRON MODIFY START
    var enable;

    enable = arBandSteerEnable.get();
    if( enable == 1 )
    {
        return true;
    }
    ...
    // If MIB arrisRouterOperationalBandSteerEnable is not supported, return false.
    return false;
    // PEGATRON MODIFY END
}
...
function resetNow() {
    // ARRIS CHANGE START - PD53380
    docsDevResetNow.set(1,"", true);
    // ARRIS CHANGE END - PD53380
    clearLoginCredential();
    logout();
}
...
//PEGATRON ADD START for PD 56157
function macAddrMask() {
    var o = formatter();
    o.load = function(v) {
        return hexToMac(v);
    };
    ...
    o.validate = function(v) {
        if (!v)
            return v;
        var reg = /^[Ff][0Ff]:[0Ff][0Ff]:[0Ff][0Ff]:[0Ff][0Ff]:[0Ff][0Ff]:[0Ff][0Ff]$/;
        if (!reg.test(v) || v.indexOf("0:F")!=-1 || v.indexOf(":0F")!=-1)
            throw xlate("Invalid MAC mask address: Must be 6 pairs of hexadecimals separated by ':' and only allow set 0 or F e.g. FF:FF:FF:00:00:00 or FF:FF:FF:FF:00:00 or FF:FF:FF:FF:FF:00");
        return v;
    };
    return o;
}
//PEGATRON ADD END for PD 56157
...
function domainNameCheck()
{
    ...
    o.validate = function(v) {
        if (v.length == 0)
        {
            throw xlate ("Target: must have a value");
        }
        else
        {
            if ( !checkIpV4Addr(v) && !checkIpV6Addr(v) )
            {  
                var r = /^([^<>|&`;\t\n](?!\$\())+$/;
                if (!r.test(v))
                {
                    throw xlate("Please input valid Address.");
                }
            }
        }
        return v;
...
function ssid() {
    ...
    o.validate = function(s) {
        ...
        //});
        // UNIHAN DEL END
        // ARRIS ADD START - PD34010
        if (isAltice())
        {
            var regex = /^[a-zA-Z0-9 ._-]+$/;
            if (!s.match(regex))
                throw xlate("Invalid SSID: Should contain 'a-z A-Z 0-9 -_. ' only.");
        }
        // ARRIS ADD END - PD34010
        return s;
    }
    return o;
}
...
function getChannels(band){
    ...
    if (band==2) {		        
		var DFSEn = arWiFi50GUIDFSEn.get();
		if (country == "jp")
		{
		    if (DFSEn == 1)
		    {
		        ch = [36,40,44,48,52,56,60,64,100,104,108,112,116,132,136,140,184,188,192,196];
		    }
		    else
		    {
		...
		else if (country == "eu" || country == "de")//	Europe
		{
			if (DFSEn == 1) 
			{
		...
		else if (country == "ar")//Argentina
		{
			if (DFSEn == 1) 
			{
			   ch = [52,56,60,64,100,104,108,112,116,132,136,140,144,149,153,157,161,165];
			}
			else
			{
				ch = [52,56,60,64,149,153,157,161,165];
			}
		}
		else if (country == "mx" || country == "br" || country == "ca")// //ARRIS ADD
		{
			if (DFSEn == 1)
			{
				ch = [36,40,44,48,52,56,60,64,100,104,108,112,116,132,136,140,144,149,153,157,161,165];
			}
			else
			{
		...
		else if (country == "bo" || country == "co" || country == "us" || country == "pa" || country == "pe" || 
				 country == "cr" || country == "cl" || country == "za" || country == "as" || country == "aw" ||
				 country == "bs" || country == "ai" || country == "dm" || country == "gd" || country == "gu" ||
				 country == "gy" || country == "jm" || country == "mp" || country == "kn" || country == "pr" ||
				 country == "lc" || country == "vc" || country == "ws" || country == "th" || country == "tc" ||
				 country == "vn" || country == "vi" || country == "ai" || country == "ag" || country == "cw" )////ARRIS ADD
		{
			if (DFSEn == 1)
			{
				ch = [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,144,149,153,157,161,165];
			}
			else
			{
				 ch = [36,40,44,48,149,153,157,161,165];
		...
		else if (country == "ni" || country == "py" || country == "hn" || country == "sv" || country == "gt")//ARRIS ADD
		{
			 if (DFSEn == 1)
			 {
				 ch = [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,144,149,153,157,161,165]; 
			 ...
			 {
				 ch = [36,40,44,48,52,56,60,64,149,153,157,161,165]; //only 100-144 is dfs channel
			 }
		}
		else if (country == "ec") // Don't support 144
		{
			if (DFSEn == 1)
			{
			...
			{
				ch = [36,40,44,48,52,56,60,64,149,153,157,161,165];
			}
		}
		else if (country == "tt") 
		{
		...
		else
		{
				if (DFSEn == 1)
				{
					ch = [36,40,44,48,52,56,60,64,100,104,108,112,116,132,136,140,144,149,153,157,161,165]; //DF Country Code 
				}
				else
				{
					ch = [36,40,44,48,149,153,157,161,165];
        ...
        if (DFSEn == 1)
        {
            var WeatherDFSChanEnable = arWiFi50WeatherDFSChanEnable.get();

            // PD 80908 remove weather radar DFS channels (116, 120, 124, 128, 132)
            // Weather radar channels: 120, 124, 128
            // For BW 80MHz, channels 116, 132 will affect weather radar channels too.
            if (WeatherDFSChanEnable != 1)
            {
                var ch_filtered = [];
                var subch = [116,120,124,128,132]; // This channel list came from requirement of PD 80908

                for (var i=0; i < ch.length; i++)
                {
                    var ignored_ch = 0;

                    for (var j = 0; j < subch.length; j++)
                    {
                        if (ch[i] == subch[j])
                        {
                            ignored_ch = 1;
                    ...
                    if (!ignored_ch)
                    {
                        ch_filtered = ch_filtered.concat([ch[i]]);
                    }
                }
        ...
        var excludeChannelEnable = arWiFiExcludeChannelEnable.get(); //PEGATRON ADD for PD 82257

        //PD77698 MOD START FOR Country "CO"
        var	isMax11Country = country ? "us,df,ca,za,as,ai,aw,bs,gd,gu,mp,kn,lc,vc,tt,vi,th,ws,tw,ag,cw,hc".contains(country) : false;
        //PD77698 MOD END
        if( isLGI() && !isVM() )
        {
            isMax11Country = true;
		...
		{
            //PEGATRON MOD START for PD 82257  
            if ( true != excludeChannelEnable )
            {          
	            ch = ch.concat([12,13]);
            }
            //PEGATRON MOD END for PD 82257 
		}            
    }
    return ch;
}
...
function timeServerCheck()
{
    var o = formatter();
    o.validate = function(v) {
        if(v.length == 0)
        {
            throw xlate("Time Server must have a value");
        }
        else
        {
            if( !checkIpV4Addr(v) && !checkIpV6Addr(v) ) //Check if input time server as ip address.
            {
                var reg=/^([^<>|&`;\t\n](?!\$\())+$/;
                if(!reg.test(v))  //Check if the input time server as host name is valid. Block special char as '<','>','|','&','`',';','tab','newline','$('
                {
                    throw xlate("Please input valid Time Server Address.");
                }
            }
        }
        return v;
...
//ADDED FOR AHNC TAB ENABLE AND DISABLE
function isAhncEnable()
{
	if(arhncMibObjectshncEnable.get() == 1){
        return true;
    }
    else {
        return false;
...
//PD86572 ADD START
function EnablePlumeAgent()
{
	if(arDevPartnerPlumeSONAdminStatus.get() == 1){
        return true;
    }
    else {
        return false;
...
function BandSteer(partURL, nodeMac, value) {
    jQuery.ajax({       
        url: "ahnc/" + partURL + "?" + "mac=" + nodeMac +"&"+ value,
        dataType:"text",
        async:true,
        cache:false    
    });
...
function isAhncSteerEnable()
{
	if(arhncMibObjectshncClientSteerEnable.get() == 1){
        return true;
    }
    else {
        return false;
...
function isDemoConfigEnabled() {
	if( ((arhncMibObjectshncDemoConfigEnable.get() == 1) || (arhncMibObjectshncDiagSystemEnable.get() == 1)) && 
		(arhncMibObjectshncTopologyEnable.get() == 1) && 
		(arhncMibObjectshncClientSteerEnable.get() == 1) ){
        return true;
    }
    else {
        return false;
...
function loadJSONData (url, key, elements, callback) {
    var obj = getJSONData(url);
    var JSONtableData = obj[key];
    JSONtableData.forEach(function(tableRow){
        var rowItem = [];
        elements.forEach(function(item) {
            rowItem.push(tableRow[item]);
        });
        callback(rowItem);
    })
}

function chkDescValid(p)
{
    var regex = /[~`!@#\$%\^\&\*\(\)\-\+\=\\\|\{\}\[\]\"\':;\?\/\.>,< ]/;
    if (!regex.test(p)) {
...
function isTopologyEnabled() {
	if(arhncMibObjectshncTopologyEnable.get() == 1){
        return true;
    }
    else {
        return false;
